...
contract BiswapLottery is ReentrancyGuard, IBiswapLottery, Ownable {
    ...
    nonReentrant
    {
        ...
        uint amountToDistribute = _lotteries[_lotteryId].amountCollectedInBSW;
        // Calculate the finalNumber based on the randomResult generated by ChainLink's fallback
        uint32 finalNumber = randomGenerator.viewRandomResult();
        uint ticketsCountPerBrackets = 0;
        uint bswSumPerBrackets = 0;
        uint winningPoolPerBracket = 0;
        for (uint i = 0; i < 6; i++){
            winningPoolPerBracket = _bswPerBracket[i] * _countTicketsPerBracket[i];
        ...
        uint256 amountToInjectionToNextLottery = amountToDistribute - bswSumPerBrackets;
        ...
        if (_autoInjection) {
            pendingInjectionNextLottery = amountToInjectionToNextLottery;
        } else {
            bswToken.safeTransfer(treasuryAddress, amountToInjectionToNextLottery);
        }
