...
contract BiswapLottery is ReentrancyGuard, IBiswapLottery, Ownable {
        ...
        nonReentrant
    {
        ...
        uint amountToDistribute = withdrawBurnAndCompetition(_lotteryId) + pendingInjectionNextLottery;
     ...
     */
    function withdrawBurnAndCompetition(uint _lotteryId) internal returns(uint){
        require(_lotteries[_lotteryId].status == Status.Close, "Lottery not close");
     ...
     */
    function chekPriceDifference(uint256 _newPrice, uint256 _oldPrice, uint _maxDiff) internal pure returns(bool diff){
